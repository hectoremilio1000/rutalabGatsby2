...
const ContainerOrdenesClients = () => {
  ...
  const fetchOrdenesPacientes = async () => {
    console.log("obteniendo las ordenes de los pacientes");
    setTableData(DataOrdenesToday);
    // try {
  ...
  useEffect(() => {
    fetchOrdenesPacientes();
  }, []);

  return (
    <>
      <div>
        <h1 className="welcome-title">Bienvenido Jampier</h1>
        <h3 className="title-ordenes">Tus ordenes</h3>
        <MaterialTable
          columns={columns}
          data={tableData}
...
export default ContainerOrdenesClients;
